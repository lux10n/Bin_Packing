...
def generate_partitions(item):
    ...
    for i in range(len(item)):
        ...
        generated+=perm
        # for permu_single in perm:
        #     for j in generated:
        #         if sum(permu_single)==sum(j):
        #             good=False
        #             break
        #     if good:generated.append(permu_single)
    return generated
            ...
            for perm in perms:
                ...
                for other_bag in tmp:
                    if sum(other_bag)<maxcap:
                        candidate_l=generate_partitions(other_bag)
...
def bin_packing(data,maxcap):
    ...
    while repeats<25:
        ...
        if nbh==[]:break
        min_nbh=get_min(nbh)
        if get_score(solution)<get_score(min_nbh):
            min_nbh=solution
        if min_nbh==solution:
            break
        if get_score(solution)==prev_score:
            repeats+=1
        else:
            repeats=0
            print('Taille de la solution optimale : {} sacs'.format(len(min_nbh)))
    if get_score(min_nbh)<get_score(solution):
        solution=min_nbh
